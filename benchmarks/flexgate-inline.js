/**
 * benchmarks/flexgate-inline.js
 *
 * Entry-point k6 script — FlexGate inline proxy scenario.
 *
 * Topology:
 *   k6 ──► HAProxy (:9003) ──► FlexGate (:8080) ──► Echo Server (:9000)
 *
 * FlexGate sits inline; every request transits its full middleware stack.
 * Overhead delta vs haproxy isolates FlexGate's cost.
 *
 * Run standalone:
 *   k6 run --out json=- benchmarks/flexgate-inline.js
 *
 * Via runner:
 *   node benchmarks/run.js --scenario flexgate-inline
 */
...
export function setup() {
  ...
  if (probe.status !== 200) {
    throw new Error(
      `[flexgate-inline] Target unreachable: ${TARGET}/health → HTTP ${probe.status}`
    );
  }
