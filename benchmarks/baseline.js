 ...
 *
 * Entry-point k6 script — Baseline (direct to echo server, no proxy).
 *
 * Topology:  k6 ──► Echo Server (:9000)
 *
 * This file is the thin wrapper used by the real-time benchmark runner
 * (benchmarks/run.js).  All logic lives in k6/scenarios/baseline.js;
 * this module simply re-exports it so the runner can reference scripts
 * from a flat directory without touching the internal scenario tree.
 *
 * Run standalone:
 *   k6 run --out json=- benchmarks/baseline.js
 *
 * Via runner:
 *   node benchmarks/run.js --scenario baseline
 */

// ── Target configuration ───────────────────────────────────────────────────
// Override with:  TARGET_URL=http://host:port k6 run benchmarks/baseline.js
...
const PROFILE  = __ENV.PROFILE    || 'ci';          // default to 'ci' for quick runs
...
export function setup() {
  ...
  if (probe.status !== 200) {
    throw new Error(
      `[baseline] Target unreachable: ${TARGET}/health → HTTP ${probe.status}`
    );
  }
