 ...
 * benchmarks/nginx.js
 *
 * Entry-point k6 script — Nginx reverse proxy scenario.
 *
 * Topology:  k6 ──► Nginx (:9001) ──► Echo Server (:9000)
 *
 * Run standalone:
 *   k6 run --out json=- benchmarks/nginx.js
 *
 * Via runner:
 *   node benchmarks/run.js --scenario nginx
 */
...
export function setup() {
  ...
  if (probe.status !== 200) {
    throw new Error(
      `[nginx] Target unreachable: ${TARGET}/health → HTTP ${probe.status}`
    );
  }
