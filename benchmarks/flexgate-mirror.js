/**
 * benchmarks/flexgate-mirror.js
 *
 * Entry-point k6 script — FlexGate mirror (async copy) scenario.
 *
 * Topology:
 *   k6 ──► FlexGate (:8081) ──► Echo Server (:9000)
 *              └──► Mirror tap (async, fire-and-forget)
 *
 * FlexGate mirrors traffic asynchronously; the critical path cost should
 * be near-zero compared to the inline scenario.
 *
 * Run standalone:
 *   k6 run --out json=- benchmarks/flexgate-mirror.js
 *
 * Via runner:
 *   node benchmarks/run.js --scenario flexgate-mirror
 */
...
export function setup() {
  ...
  if (probe.status !== 200) {
    throw new Error(
      `[flexgate-mirror] Target unreachable: ${TARGET}/health → HTTP ${probe.status}`
    );
  }
